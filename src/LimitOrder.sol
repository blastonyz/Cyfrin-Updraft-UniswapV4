...
contract LimitOrder is TStore {
    ...
    function unlockCallback(
        ...
        if (action == ADD_LIQUIDITY) {
            ...
            if (zeroForOne) {
                ...
                amountToPay = uint256((-amount0).toUint128());
            } else {
                require(amount0 == 0 && amount1 < 0, "Tick crossed");
                currency = key.currency1;
                amountToPay = uint256((-amount1).toUint128());
            }
    ...
    function cancel(
        ...
        if (bucket.liquidity > 0) {
            ...
            if (amount0 > fee0) {
                CurrencyLib.transferOut(Currency.unwrap(key.currency0), msg.sender, amount0 - fee0);
            }
            if (amount1 > fee1) {
                CurrencyLib.transferOut(Currency.unwrap(key.currency1), msg.sender, amount1 - fee1);
            }
        } else {
            amount0 += bucket.amount0;
            bucket.amount0 = 0;
            if (amount0 > 0) {
                CurrencyLib.transferOut(Currency.unwrap(key.currency0), msg.sender, amount0);
            }
            amount1 += bucket.amount1;
            bucket.amount1 = 0;
            if (amount1 > 0) {
                CurrencyLib.transferOut(Currency.unwrap(key.currency1), msg.sender, amount1);
            }
        }
